...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('products', function (Blueprint $t) {
            if (!Schema::hasColumn('products', 'sku')) {
                $t->string('sku', 64)->unique()->after('id');
            }
            if (!Schema::hasColumn('products', 'name')) {
                $t->string('name')->after('sku');
            }
            if (!Schema::hasColumn('products', 'description')) {
                $t->text('description')->nullable()->after('name');
            }
            if (!Schema::hasColumn('products', 'category')) {
                $t->string('category', 64)->nullable()->after('description');
            }
            if (!Schema::hasColumn('products', 'uom')) {
                $t->string('uom', 32)->nullable()->after('category');
            }
            if (!Schema::hasColumn('products', 'reorder_point')) {
                $t->integer('reorder_point')->default(0)->after('uom');
            }
            if (!Schema::hasColumn('products', 'is_active')) {
                $t->boolean('is_active')->default(true)->after('reorder_point');
            }
        });
    }
    ...
    public function down(): void
    {
        Schema::table('products', function (Blueprint $t) {
            ...
            if (Schema::hasColumn('products', 'name'))          $t->dropColumn('name');
